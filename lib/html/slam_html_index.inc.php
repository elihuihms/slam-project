...
EOL;
	/* link to dashboard */
	if ($request->location == 'dash')
		$s.="<tr><td class='categoryListElementSelected' style='padding-bottom:5px'><a href='".$request->makeRequestURL($config,array('location'=>'dash'),false)."'>Dashboard</a> &raquo;</td></tr>\n";
	else
		$s.="<tr><td class='categoryListElement' style='padding-bottom:5px'><a href='".$request->makeRequestURL($config,array('location'=>'dash'),false)."'>Dashboard</a> &raquo;</td></tr>\n";
	
	$request_categories = array_keys($request->categories);
	foreach($categories as $category)
	{
		if(($request->location == 'list') && ($request_categories[0] == $category))
			$s.="<tr><td class='categoryListElementSelected'><a href='".$request->makeRequestURL($config,array('location'=>'list','category'=>array($category)),false)."'>$category</a> &raquo;</td></tr>\n";
		else
			$s.="<tr><td class='categoryListElement'><a href='".$request->makeRequestURL($config,array('location'=>'list','category'=>array($category)),false)."'>$category</a> &raquo;</td></tr>\n";
	}
